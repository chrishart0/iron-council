...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["agent_player_id"],
            ["players.id"],
            name=op.f("fk_owned_agent_guidance_agent_player_id_players"),
        ),
        sa.ForeignKeyConstraint(
            ["match_id"],
            ["matches.id"],
            name=op.f("fk_owned_agent_guidance_match_id_matches"),
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_owned_agent_guidance")),
    )
    op.create_index(
        op.f("ix_owned_agent_guidance_agent_player_id"),
        "owned_agent_guidance",
        ["agent_player_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_owned_agent_guidance_owner_user_id"),
        "owned_agent_guidance",
        ["owner_user_id"],
        unique=False,
    )
...
def downgrade() -> None:
    for index_name in _drop_indexes():
        op.drop_index(index_name, table_name="owned_agent_guidance")
    op.drop_table("owned_agent_guidance")
