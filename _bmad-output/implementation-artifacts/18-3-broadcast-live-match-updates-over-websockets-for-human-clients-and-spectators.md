...
- Follow-up fix: expanded websocket payloads so players receive direct/group chat state relevant to them and spectators receive the full all-chat spectator view promised by the architecture docs.
- Follow-up fix: triggered websocket refresh broadcasts for direct message writes, group-chat creation, group-chat message writes, and command-envelope message writes so connected clients stay current after chat-visible events.
- Follow-up fix: aligned the canonical websocket contract around `/ws/match/{match_id}` plus `token`, while preserving `/ws/matches/{match_id}` and `api_key` as compatibility aliases.
- Added API-level websocket tests for initial payload shape, connection lifecycle, invalid viewer/auth cases, compatibility aliases, public/private chat refresh, and a real-process websocket smoke that proves both a player and a spectator receive a live tick-driven update.
