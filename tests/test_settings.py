...
def test_get_settings_loads_database_url_from_local_env_file(tmp_path: Path) -> None:
    ...
    assert settings.database_url == "postgresql://file-user:file-password@127.0.0.1:54321/file_db"
...
def test_get_settings_prefers_explicit_environment_over_local_env_file(tmp_path: Path) -> None:
    ...
    assert settings.database_url == "postgresql://env-user:env-password@127.0.0.1:54321/env_db"
...
def test_get_settings_uses_configured_env_file_override(tmp_path: Path) -> None:
    ...
    assert (
        ...
        == "postgresql://override-user:override-password@127.0.0.1:54321/override_db"
    )
